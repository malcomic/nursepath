...
export class GuideService {
  ...
  async getGuidesByCategory(categoryId: string) {
    await categoryRepository.findById(categoryId); // Verify category exists
  ...
  async updateGuide(
    ...
    if (data.categoryId) {
      await categoryRepository.findById(data.categoryId); // Verify category exists
    }

    return guideRepository.update(id, data as any);
  }
