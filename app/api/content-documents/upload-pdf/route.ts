...
import { putLibraryPdf } from '@/lib/blob/library-private';
...
export const POST = withHandler(async (req) => {
  ...
  const blob = await putLibraryPdf(
    `content/pdf/${timestamp}-${safeName}`,
    bytes,
    file.type
  );
  ...
  if (previewBytes) {
    const previewBlob = await put(
      `content/preview/${timestamp}-${safeName}`,
  ...
  return jsonResponse({
    ...
    data: { fileUrl: blob.url, previewPdfUrl },
  });
});
